...
class MainActivity : ComponentActivity() {
    ...
    private fun applyExternalPhoneDecision(local: ClientRecord, contact: PhoneContact, detected: PhoneEntry, label: String, replaceIndex: Int?) {
        ...
        val detectedKey = normalizePhone(detected.number)
        val duplicateIndex = phones.indexOfFirst { normalizePhone(it.number) == detectedKey }
        if (replaceIndex == null && detectedKey.isNotBlank() && duplicateIndex >= 0) {
            showStatus("This telephone number is already in the Buy & Sell client record. No duplicate was added.")
            return
        }
    ...
    private fun showClientEditScreen(contact: PhoneContact) {
        ...
        save.setOnClickListener {
            ...
            validateUniquePhoneNumbers(finalPhones)?.let { message ->
                AlertDialog.Builder(this)
                    .setTitle("DUPLICATE TELEPHONE NUMBER")
                    .setMessage(message)
                    .setPositiveButton("OK", null)
                    .show()
                save.isEnabled = true
    ...
    private fun syncEditedClientToGoogle(
        ...
        Thread {
            try {
                ...
                val mergedPhones = linkedMapOf<String, JSONObject>()
                phones.filter { it.number.isNotBlank() }.forEach { phone ->
                    val key = normalizePhone(phone.number)
                    if (key.isNotBlank()) mergedPhones[key] = JSONObject().apply {
                        put("value", phone.number.trim())
                        put("type", googlePhoneType(phone.label.trim()))
                    }
                }
                val desiredPhones = JSONArray().apply { mergedPhones.values.forEach { put(it) } }
                ...
                // Post-PATCH verification: HTTP 200 alone is not proof that the desired
                // telephone list is what Google now stores.
                val verifiedPerson = fetchGooglePerson(resourceName, accessToken)
                val actualKeys = linkedSetOf<String>()
                val actualPhones = verifiedPerson.optJSONArray("phoneNumbers")
                if (actualPhones != null) {
                    for (i in 0 until actualPhones.length()) {
                        val value = actualPhones.optJSONObject(i)?.optString("value").orEmpty()
                        val key = normalizePhone(value)
                        if (key.isNotBlank()) actualKeys.add(key)
                    }
                }
                val desiredKeys = linkedSetOf<String>()
                phones.filter { it.number.isNotBlank() }.forEach {
                    val key = normalizePhone(it.number)
                    if (key.isNotBlank()) desiredKeys.add(key)
                }
                if (actualKeys != desiredKeys) {
                    throw IllegalStateException("Google verification failed: expected ${desiredKeys.size} phone number(s), found ${actualKeys.size}.")
                }
    ...
    private fun undoClear() {
        ...
        try {
            ...
            setExistingClientCaptureMode(restoredContact != null)
    ...
    private fun saveOrUpdate() {
        ...
        val now = System.currentTimeMillis()
        val existingRecord = selected?.let { clientDb.getClientByPhoneContactId(it.id.toString()) }
        // When an existing client is selected, the Capture fields are hidden.
        // Preserve the authoritative denormalised values rather than writing stale
        // hidden-field contents alongside the merged phones_json list.
        val effectiveMainPhone = existingRecord?.mainPhone ?: mainPhone
        val effectiveAdditionalPhone = existingRecord?.additionalPhone ?: additionalPhone
        val effectiveAdditionalPhoneLabel = existingRecord?.additionalPhoneLabel ?: additionalPhoneLabel
        val effectiveWhatsapp = existingRecord?.whatsapp ?: whatsapp
        ...
        val savedId = clientDb.saveOrUpdateClient(
            ...
            surname = surname,
            mainPhone = effectiveMainPhone,
            additionalPhone = effectiveAdditionalPhone,
            additionalPhoneLabel = effectiveAdditionalPhoneLabel,
        ...
        if (local.identifyingName != identifyingName ||
            ...
            local.mainPhone != effectiveMainPhone ||
            local.additionalPhone != effectiveAdditionalPhone ||
            local.additionalPhoneLabel != effectiveAdditionalPhoneLabel ||
            local.whatsapp != effectiveWhatsapp ||
    ...
    private fun updateExistingGoogleContact(
        ...
        Thread {
            ...
            try {
                ...
                val mergedPhoneMap = linkedMapOf<String, JSONObject>()
                phones.filter { it.number.isNotBlank() }.forEach { phone ->
                    val key = normalizePhone(phone.number)
                    if (key.isNotBlank()) mergedPhoneMap[key] = JSONObject().apply {
                        put("value", phone.number.trim())
                        put("type", googlePhoneType(phone.label))
                    }
                }
                val mergedNumbers = JSONArray().apply { mergedPhoneMap.values.forEach { put(it) } }
    ...
    private class ClientDb(context: android.content.Context) : SQLiteOpenHelper(context, "buyandsell.db", null, 9) {
        ...
        // Phone normalization is needed inside ClientDb as well as MainActivity.
        // ClientDb is a nested (non-inner) class, so it cannot access MainActivity's
        // private helper directly. Keep this local helper to avoid unresolved references.
        private fun normalizePhone(value: String): String = value.filter { it.isDigit() }

        fun findPossibleDuplicate(identifyingName: String, mainPhone: String): ClientRecord? {
            val db = readableDatabase
            val normalizedPhone = normalizePhone(mainPhone)
            if (normalizedPhone.isNotBlank()) {
                db.query("clients", arrayOf("id", "main_phone", "additional_phone", "whatsapp"), null, null, null, null, "id ASC").use { cursor ->
                    while (cursor.moveToNext()) {
                        val main = normalizePhone(cursor.getString(cursor.getColumnIndexOrThrow("main_phone")))
                        val additional = normalizePhone(cursor.getString(cursor.getColumnIndexOrThrow("additional_phone")))
                        val whatsapp = normalizePhone(cursor.getString(cursor.getColumnIndexOrThrow("whatsapp")))
                        if (normalizedPhone.isNotBlank() && (normalizedPhone == main || normalizedPhone == additional || normalizedPhone == whatsapp)) {
                            return getClient(cursor.getLong(cursor.getColumnIndexOrThrow("id")))
                        }
                    }
                }
            }
    ...
    private fun validateUniquePhoneNumbers(phones: List<EditablePhone>): String? {
        val seen = mutableSetOf<String>()
        phones.filter { it.number.isNotBlank() }.forEach { phone ->
            val key = normalizePhone(phone.number)
            if (key.isNotBlank() && !seen.add(key)) {
                return "Duplicate telephone number detected: ${phone.number}. Each telephone number may be saved only once per client, even when labels differ."
            }
        }
        return null
    }
