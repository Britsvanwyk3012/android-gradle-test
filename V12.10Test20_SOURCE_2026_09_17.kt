    ...
    private fun syncEditedClientToGoogle(
        ...
        Thread {
            try {
                ...
                val livePhones = current.optJSONArray("phoneNumbers")
                if (livePhones != null) {
                    for (i in 0 until livePhones.length()) {
                        val live = livePhones.optJSONObject(i) ?: continue
                        val value = live.optString("value").trim()
                        val key = normalizePhone(value)
                        if (key.isNotBlank() && !mergedPhones.containsKey(key)) {
                            mergedPhones[key] = JSONObject().apply {
                                put("value", value)
                                put("type", live.optString("type").ifBlank { "other" })
                            }
                        }
                    }
                }
    ...
    private fun applyAndroidGoogleRawContactUpdate(contactId: Long, name: String, surname: String, phones: List<EditablePhone>) {
        ...
        Thread {
            try {
                ...
                val mergedPhoneEntries = linkedMapOf<String, EditablePhone>()
                phones.filter { it.number.isNotBlank() }.forEach { phone ->
                    val key = normalizePhone(phone.number)
                    if (key.isNotBlank()) mergedPhoneEntries[key] = phone
                }
                contentResolver.query(
                    ...
                    while (cursor.moveToNext() && numberIndex >= 0) {
                        ...
                        if (key.isNotBlank() && !mergedPhoneEntries.containsKey(key)) {
                            val label = if (labelIndex >= 0) cursor.getString(labelIndex).orEmpty().ifBlank { "New" } else "New"
                            mergedPhoneEntries[key] = EditablePhone(number, label)
                        }
                    }
                }
                ...
                mergedPhoneEntries.values.forEach { phone ->
                ...
                Log.e("BuyAndSell", "Android Contacts update failed", error)
                runOnUiThread { showStatus("WARNING — Local record saved, but Android Contacts could not be updated. Please check permissions and retry.") }
            }
        }.start()
    }
    ...
    private fun updateExistingGoogleContact(
        ...
        Thread {
            ...
            try {
                ...
                val livePhoneNumbers = current.optJSONArray("phoneNumbers")
                if (livePhoneNumbers != null) {
                    for (i in 0 until livePhoneNumbers.length()) {
                        val live = livePhoneNumbers.optJSONObject(i) ?: continue
                        val value = live.optString("value").trim()
                        val key = normalizePhone(value)
                        if (key.isNotBlank() && !mergedPhoneMap.containsKey(key)) {
                            mergedPhoneMap[key] = JSONObject().apply {
                                put("value", value)
                                put("type", live.optString("type").ifBlank { "other" })
                            }
                        }
